...
class UtilsPanel(gui.NotebookPanel):  # Polemos: changes and fixes.
    ...
    def __init__(self, parent):
        ...
        self.gDescription = wx.TextCtrl(
            right, -1, style=wx.TE_MULTILINE | wx.TE_READONLY
        )
    ...
    def OnShow(self):
        ...
        if mosh.UtilsData.refresh():
            singletons.utilsList.RefreshUI()  # This is selfless, don't self it.
        self.SetStatusCount()
...
class UtilsList(gui.List):  # Polemos: Changes and optimizations.
            ...
            if mode and (colDex == 0):
                try:
                    ...
                    self.list.InsertStringItem(
                        itemDex, value.decode("utf-8", errors="ignore")
                    )
            else:
                try:
                    ...
                    try:
                        ...
                        try:
                            ...
                            self.list.SetStringItem(
                                itemDex, colDex, value.encode("utf-8")
                            )
        # --Selection State
        if item in selected:
    ...
    def SortItems(
        self, col=None, reverse=-2
    ):  # Polemos: new ID and flag implementation.
        """Sort Items"""
        ...
        if col == "File":
            ...
            raise mosh.SortKeyError(
                _("Unrecognized sort key: %s" % col)
            )  # Polemos: fix
        # --Ascending
        if reverse:
            self.items.reverse()
    ...
    def OnItemSelected(
        self, event=None
    ):  # Polemos: This is Unicode kebab, not sandwich.
        """..."""
        ID = event.GetText()
    ...
    def ModifyItem(self):  # Polemos: Unicode edits.
        ...
        while idx < len(names):
            name = names[idx]
            try:
                dialog = gui.dialog.UtilsDialog(
                    self, new=False, data=((name,) + self.data[name])
                )
            except:
                dialog = gui.dialog.UtilsDialog(
                    self, new=False, data=((name.decode("utf-8"),) + self.data[name])
                )
            if dialog.ShowModal() != wx.ID_OK:
                dialog.Destroy()
                return
