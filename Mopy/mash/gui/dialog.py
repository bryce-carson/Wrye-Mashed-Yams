...
def chkChars(text_val, strict=False):
    ...
    for x in nope:
        if not strict:
            text_val = text_val.replace(x, "_")
        else:
            text_val = text_val.replace(x, "")
...
class GaugeDialog(wx.Dialog):  # Polemos
    ...
    def __init__(
        self, parent, message=_("Please wait..."), title=_("Progress..."), max=100
    ):
        """Init."""
        wx.Dialog.__init__(
            self,
        ...
        if True:  # Contents
            ...
            self.message = wx.StaticText(
                self.mainPanel, wx.ID_ANY, message, dPos, dSize, 0
            )
            self.gauge = wx.Gauge(
                self.mainPanel,
                wx.ID_ANY,
        ...
        if True:  # Layout
            ...
            contentsSizer.AddMany(
                [
                    (self.message, 0, wx.EXPAND | wx.TOP | wx.RIGHT | wx.LEFT, 10),
                    ((0, 0), 1, wx.EXPAND, 5),
                    (self.gauge, 0, wx.EXPAND | wx.ALL, 10),
                ]
            )
...
class netProgressDialog:  # Polemos
    ...
    def __init__(
        ...
        message=_("Checking for new version..."),
        maximum=5,
        parent=None,
    ):
        """Init."""
...
class ConflictDialog(wx.Dialog):  # Polemos
    ...
    def __init__(self, parent, modName, title=_("Target folder already exists...")):
        ...
        if True:  # Contents
            ...
            self.warn_bmp = wx.StaticBitmap(
                self.panel,
                wx.ID_ANY,
                wx.Bitmap("images/warning.png", wx.BITMAP_TYPE_ANY),
                dPos,
                dSize,
                0,
            )
            self.textQ = wx.StaticText(
                self.panel,
                wx.ID_ANY,
                _(
                    "A mod folder with the same name already exists. How do you wish to proceed?"
                ),
                dPos,
                Size(-1, -1),
                0,
            )
            self.textMod = wx.TextCtrl(
                self.panel,
                wx.ID_ANY,
                modName,
                dPos,
            ...
            self.chkBox = wx.CheckBox(
                self, wx.ID_ANY, _("Backup existing mod..."), dPos, dSize, 0
            )
            # Buttons
            self.rename_btn = wx.Button(
                self.panel, wx.ID_ANY, _("Rename mod:"), dPos, Size(95, 22), 0
            )
            self.overwrite_btn = wx.Button(
                self, wx.ID_ANY, _("Overwrite..."), dPos, Size(85, 22), 0
            )
            self.replace_btn = wx.Button(
                self, wx.ID_ANY, _("Replace..."), dPos, Size(75, 22), 0
            )
            self.cancel_btn = wx.Button(
                self, wx.ID_CANCEL, _("Cancel"), dPos, Size(55, 22), 0
            )
        ...
        if True:  # Layout
            ...
            modSizer.AddMany(
                [
                    (self.rename_btn, 0, wx.ALL, 5),
                    (self.textMod, 1, wx.ALL | wx.ALIGN_CENTER, 5),
                ]
            )
            textSizer = wx.BoxSizer(wx.VERTICAL)
            textSizer.AddMany([(self.textQ, 1, wx.ALL, 5), (modSizer, 0, wx.EXPAND, 5)])
            contentSizer = wx.BoxSizer(wx.HORIZONTAL)
            contentSizer.AddMany(
                [
                    (
                        self.warn_bmp,
                        1,
                    ...
                    (textSizer, 1, wx.EXPAND | wx.ALL, 5),
                ]
            )
            ...
            btnSizer.AddMany(
                [
                    (self.chkBox, 0, wx.ALIGN_CENTER_VERTICAL | wx.ALL, 5),
                    ((0, 0), 1, wx.EXPAND, 5),
                    (self.overwrite_btn, 0, wx.ALL, 5),
                    (self.replace_btn, 0, wx.ALL, 5),
            ...
            mainSizer.AddMany(
                [
                    (self.panel, 1, wx.EXPAND, 5),
                    (btnSizer, 0, wx.EXPAND | wx.LEFT | wx.RIGHT, 5),
                ]
            )
            self.SetSizer(mainSizer)
            self.Layout()
...
def DirDialog(parent, message=_("Choose a directory."), defaultPath=""):  # Polemos
    ...
    with wx.DirDialog(
        parent, message, defaultPath, style=wx.DD_NEW_DIR_BUTTON
    ) as dialog:
        dialog.ShowModal()
        path = dialog.GetPath()
...
def ContinueQuery(
    parent, tmessage, message, continueKey, title=_("Warning"), nBtn=True
):  # Polemos
    ...
    dialog = wx.Dialog(
        parent, id=wx.ID_ANY, title=title, pos=dPos, size=wx.Size(-1, -1), style=style
    )
    ...
    okBtn = wx.Button(dialog, wx.ID_OK, _("OK" if not nBtn else "Yes"), dPos, dSize, 0)
    show = wx.CheckBox(
        dialog, wx.ID_ANY, _(" Don't show this in the future."), dPos, dSize, 0
    )
    ...
    cntSizer.AddMany(
        [
            (title, 0, wx.ALL | wx.EXPAND | wx.ALIGN_CENTER_HORIZONTAL, 5),
    ...
    btnSizer.AddMany(
        [
            (okBtn, 0, wx.ALL, 5),
            (show, 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 5),
            (cnlBtn, 0, wx.ALL, 5),
        ]
    )
    mainSizer = wx.BoxSizer(wx.VERTICAL)
    mainSizer.AddMany(
        [
            (cntSizer, 1, wx.EXPAND | wx.RIGHT | wx.LEFT, 5),
            (btnSizer, 0, wx.ALIGN_CENTER_HORIZONTAL, 5),
        ]
    )
    ...
    if result:
        conf.settings[continueKey] = show.GetValue()
    return result
...
def LogMessage(parent, message, logText, title="", style=0, asDialog=True):
    ...
    if message:
        sizer.Add(
            wx.StaticText(window, -1, message),
            0,
...
class WelcomeDialog(wx.Dialog):  # Polemos
    ...
    def __init__(self, parent, headtext, detailstext, title=_("Welcome!!!")):
        ...
        if True:  # Contents
            bad_wrye = wx.StaticBitmap(
                panel,
                wx.ID_ANY,
                wx.Bitmap("images/wr_b_mini.png", wx.BITMAP_TYPE_ANY),
                dPos,
                dSize,
                0,
            )
        ...
        if True:  # Layout
            ...
            textSizer.AddMany(
                [
                    (headline, 0, wx.EXPAND | wx.TOP | wx.BOTTOM | wx.LEFT, 10),
                    (details, 0, wx.EXPAND | wx.TOP | wx.BOTTOM | wx.LEFT, 10),
                ]
            )
            panelSizer = wx.BoxSizer(wx.HORIZONTAL)
            panelSizer.AddMany(
                [
                    (bad_wrye, 0, wx.EXPAND | wx.LEFT, 5),
                    (textSizer, 1, wx.EXPAND | wx.RIGHT | wx.LEFT, 5),
                ]
            )
            ...
            main_sizer.AddMany(
                [(panel, 1, wx.EXPAND, 5), (buttonSizer, 0, wx.EXPAND, 5)]
            )
...
class ModInstallDialog(wx.Dialog):  # Polemos
    ...
    def __init__(
        ...
        if True:  # Contents
            ...
            self.text = wx.StaticText(
                self, wx.ID_ANY, _("Mod Name:     "), dPos, Size(-1, -1), 0
            )
            self.mod_name = wx.TextCtrl(
                self, wx.ID_ANY, "", dPos, Size(-1, 20), 0 | wx.SIMPLE_BORDER
            )
            self.mod_name.SetLabel(modname)
            # Buttons
            self.advanced_btn = wx.Button(
                self, wx.ID_ANY, _("Advanced"), dPos, Size(75, 21), 0
            )
            self.ok_btn = wx.Button(self, wx.ID_OK, _("OK"), dPos, Size(35, 21), 0)
            self.cancel_btn = wx.Button(
                self, wx.ID_CANCEL, _("Cancel"), dPos, Size(60, 21), 0
            )
        ...
        if True:  # Layout
            ...
            contentSizer = wx.BoxSizer(wx.HORIZONTAL)
            contentSizer.AddMany(
                [(self.text, 0, wx.ALIGN_CENTER, 5), (self.mod_name, 1, 0, 5)]
            )
            btnSizer = wx.BoxSizer(wx.HORIZONTAL)
            btnSizer.AddMany(
                [
                    ((0, 0), 1, 0, 5),
                    (self.advanced_btn, 0, wx.RIGHT, 5),
            ...
            mainSizer = wx.BoxSizer(wx.VERTICAL)
            mainSizer.AddMany(
                [(contentSizer, 0, wx.ALL | wx.EXPAND, 5), (btnSizer, 0, wx.EXPAND, 5)]
            )
            self.SetSizer(mainSizer)
            self.Layout()
    ...
    def OnOk(self, event):
        if isinstance(self.data_files, basestring):
...
class RenameDialog(wx.Dialog):  # Polemos
    ...
    def __init__(
        ...
        if True:  # Contents
            self.text = wx.StaticText(
                self, wx.ID_ANY, _("Please select a new mod name:"), dPos, dSize, 0
            )
            self.field = wx.TextCtrl(
                self, wx.ID_ANY, "", dPos, Size(-1, 20), 0 | wx.SIMPLE_BORDER
            )
            self.rename_btn = wx.Button(
                self, wx.ID_ANY, _("Rename"), dPos, Size(65, 22), 0
            )
            self.cancel_btn = wx.Button(
                self, wx.ID_CANCEL, _("Cancel"), dPos, Size(60, 22), 0
            )
        if True:  # Theming
        ...
        if True:  # Layout
            ...
            contentSizer.AddMany(
                [
                    (self.text, 0, wx.EXPAND | wx.ALL, 5),
                    (self.field, 0, wx.EXPAND | wx.RIGHT | wx.LEFT, 5),
                ]
            )
            btnSizer = wx.BoxSizer(wx.HORIZONTAL)
            btnSizer.AddMany(
                [(self.rename_btn, 0, wx.ALL, 5), (self.cancel_btn, 0, wx.ALL, 5)]
            )
            mainSizer = wx.BoxSizer(wx.VERTICAL)
            mainSizer.AddMany(
                [(contentSizer, 1, wx.EXPAND, 5), (btnSizer, 0, wx.ALIGN_RIGHT, 5)]
            )
            self.SetSizer(mainSizer)
            self.Layout()
        if True:  # Actions
    ...
    def onUpdate(self, event):
        ...
        if (
            self.field.GetValue() == ""
            or self.field.GetValue().lower() == self.oldName.lower()
        ):
...
def ManualDetectDialog(
    parent,
    message,
    title="",
    style=wx.YES_NO | wx.ICON_EXCLAMATION | wx.STAY_ON_TOP | wx.CANCEL,
):  # Polemos
...
def WarningQuery(
    parent, message, title="", style=(wx.YES_NO | wx.ICON_EXCLAMATION | wx.STAY_ON_TOP)
):
...
def ErrorQuery(
    parent, message, title="", style=(wx.YES_NO | wx.ICON_HAND | wx.STAY_ON_TOP)
):  # Polemos
...
def ErrorMessage(
    ...
    message,
    title=_("Error"),
    style=(wx.OK | wx.ICON_HAND | wx.STAY_ON_TOP),
    dtype="error",
    modal=True,
):
    """Shows a modal error message."""
    return (
        Message(parent, message, title, style)
        if modal
        else MessageU(parent, message, dtype, title, modal)
    )


def Message(parent, message, title="", style=wx.OK | wx.STAY_ON_TOP):
...
class MessageU(wx.Dialog):
    ...
    def __init__(
        ...
        if True:  # Content
            panel = wx.Panel(self, wx.ID_ANY, dPos, dSize, wx.TAB_TRAVERSAL)
            sign = wx.StaticBitmap(
                panel,
                wx.ID_ANY,
                wx.ArtProvider.GetBitmap(mode[dtype][0], mode[dtype][1]),
                dPos,
                dSize,
                0,
            )
            ...
            self.okBtn = wx.Button(
                self, wx.ID_OK, _("OK (%s)" % self.count), dPos, dSize, 0
            )
        ...
        if True:  # Layout
            ...
            txtSizer.AddMany(
                [
                    (headTxt, 1, wx.EXPAND | wx.ALL, 5),
                    (mainTxt, 1, wx.EXPAND | wx.ALL, 5),
                ]
            )
            ...
            mainSizer.AddMany(
                [
                    (panel, 1, wx.EXPAND | wx.BOTTOM, 5),
                    (self.okBtn, 0, wx.ALL | wx.ALIGN_RIGHT, 5),
                ]
            )
            self.SetSizer(mainSizer)
            self.Layout()
...
class MaxCharDialog(wx.Dialog):  # Polemos
    ...
    def __init__(
        ...
        """Init."""
        wx.Dialog.__init__(
            self, parent=parent, id=wx.ID_ANY, size=size, pos=pos, style=style
        )
        ...
        if True:  # Contents
            ...
            self.fld_po = wx.TextCtrl(
                captionBox, wx.ID_ANY, current, dPos, dSize, wx.TE_MULTILINE
            )
            self.fldchars = wx.TextCtrl(
                captionBox, wx.ID_ANY, self.char_remain(), dPos, dSize, wx.TE_READONLY
            )
            self.btnOK = wx.Button(captionBox, wx.ID_OK, _("OK"), dPos, (-1, 22), 0)
            btnCancel = wx.Button(
                captionBox, wx.ID_CANCEL, _("Cancel"), dPos, (-1, 22), 0
            )
        if True:  # Layout
            btnSizer = wx.BoxSizer(wx.HORIZONTAL)
            btnSizer.AddMany(
            ...
            mainSizer.AddMany(
                [(self.fld_po, 1, wx.EXPAND, 5), (btnSizer, 0, wx.EXPAND | wx.TOP, 5)]
            )
            self.SetSizer(mainSizer)
            self.Layout()
        if True:  # Events
    ...
    def onUpdate(self, event):
        """Timer events."""
        if len(self.fld_po.GetValue()) > self.maxchar:
            self.btnOK.Disable()
        else:
            self.btnOK.Enable()
...
class date_time_dialog(wx.Dialog):  # Polemos
    ...
    def __init__(
        self, parent, title, caption, datetime, id=wx.ID_ANY, size=(418, 258), pos=dPos
    ):
        """Init."""
        wx.Dialog.__init__(
            self,
            ...
            pos=pos,
            size=size,
            style=wx.CAPTION | wx.DEFAULT_DIALOG_STYLE | wx.STAY_ON_TOP,
        )
        self.datetime = datetime
        year, month, day, hour, min, sec = time.strptime(datetime, "%m-%d-%Y %H:%M:%S")[
            0:6
        ]
        ...
        if True:  # Contents
            ...
            self.month_static = wx.StaticText(
                dateBox, wx.ID_ANY, _("Month:"), dPos, dSize, 0
            )
            ...
            self.day_static = wx.StaticText(
                dateBox, wx.ID_ANY, _("Day:"), dPos, dSize, 0
            )
            self.day = wx.SpinCtrl(
                dateBox,
                wx.ID_ANY,
                "",
                dPos,
            ...
            self.year_static = wx.StaticText(
                dateBox, wx.ID_ANY, _("Year:"), dPos, dSize, 0
            )
            self.year = wx.SpinCtrl(
                dateBox,
                wx.ID_ANY,
                "",
                dPos,
            ...
            self.hour_static = wx.StaticText(
                timeBox, wx.ID_ANY, _("Hour:"), dPos, dSize, 0
            )
            self.hour = wx.SpinCtrl(
                timeBox,
                wx.ID_ANY,
                "",
                dPos,
            ...
            self.min_static = wx.StaticText(
                timeBox, wx.ID_ANY, _("Min:"), dPos, dSize, 0
            )
            self.min = wx.SpinCtrl(
                timeBox,
                wx.ID_ANY,
                "",
                dPos,
            ...
            self.sec_static = wx.StaticText(
                timeBox, wx.ID_ANY, _("Sec:"), dPos, dSize, 0
            )
            self.sec = wx.SpinCtrl(
                timeBox,
                wx.ID_ANY,
                "",
                dPos,
            ...
            self.cancel_button = wx.Button(
                self, wx.ID_CANCEL, _("Cancel"), dPos, (-1, 22), 0
            )
        ...
        if True:  # Theming
            ...
            self.month.SetBackgroundColour(
        ...
        if True:  # Layout
            ...
            dateSizer.AddMany(
                [
                    ...
                    (self.month, 0, wx.RIGHT | wx.LEFT, 5),
                    ((0, 0), 1, wx.EXPAND, 5),
                    (
                        self.day_static,
                        0,
                    ...
                    (self.year, 0, wx.RIGHT | wx.LEFT, 5),
                ]
            )
            # Time
            ...
            timeSizer.AddMany(
                [
                    ...
                    (self.hour, 0, wx.RIGHT | wx.LEFT, 5),
                    ((0, 0), 1, wx.EXPAND, 5),
                    (
                        self.min_static,
                        0,
            ...
            btnSizer.AddMany(
                [
                    (self.ok_button, 0, wx.ALL, 5),
                    (self.smplTextCtrl, 1, wx.ALIGN_CENTER_VERTICAL | wx.ALL, 5),
                    (self.cancel_button, 0, wx.ALL, 5),
                ]
            )
            # Main
            mainSizer = wx.BoxSizer(wx.VERTICAL)
            mainSizer.AddMany(
                [
                    (self.info, 0, wx.ALL | wx.EXPAND, 5),
                    (dateSizer, -1, wx.EXPAND, 5),
                    (timeSizer, -1, wx.EXPAND, 5),
                    (mnlSizer, 0, wx.EXPAND, 5),
                    (btnSizer, 0, wx.EXPAND, 5),
                ]
            )
            self.SetSizer(mainSizer)
            self.Layout()
    ...
    def onUpdate(self, event):
        """Timer actions."""
        self.smplTextCtrl.SetValue(
            self.export_date().replace(" ", ", ").replace("-", "/")
        )
...
class SimpleListDialog(wx.Dialog):  # Polemos
    ...
    def __init__(
        ...
        self.SetSizeHints(-1, -1)
        if not type(choices) is list:
            choices = [choices]
        self.choices = choices

        if True:  # Content
            self.intro = wx.StaticText(self, wx.ID_ANY, msg, dPos, dSize, 0)
            self.wxlist = wx.ListBox(
                self, wx.ID_ANY, dPos, dSize, self.choices, wx.LB_ALWAYS_SB
            )
        ...
        if True:  # Layout
            ...
            contentSizer.AddMany(
                [
                    (self.intro, 0, wx.TOP | wx.RIGHT | wx.LEFT, 5),
                    (self.wxlist, 1, wx.EXPAND | wx.ALL, 5),
                ]
            )
            btnSizer = wx.BoxSizer(wx.HORIZONTAL)
            btnSizer.AddMany(
                [
                    (self.ok_btn, 0, wx.RIGHT | wx.LEFT | wx.DOWN, 5),
                    (self.cancel_btn, 0, wx.RIGHT | wx.LEFT | wx.DOWN, 5),
                ]
            )
            mainSizer = wx.BoxSizer(wx.VERTICAL)
            mainSizer.AddMany(
    ...
    def OnOK(self, event):
        try:
            self.Selection = self.wxlist.GetSelection()
        except:
            pass
        finally:
            self.OnCancel(None)
...
class ListDialog(wx.Dialog):  # Polemos
    ...
    def __init__(self, parent, title, data):
        ...
        if True:  # Contents
            self.contents_list = wx.ListBox(
                self, wx.ID_ANY, dPos, dSize, "", wx.LB_ALWAYS_SB | wx.LB_SORT
            )
        ...
        if True:  # Buttons
            ...
            self.remove_button = wx.Button(
                self, wx.ID_ANY, _("Remove"), dPos, (60, 26), 0
            )
            self.ok_button = wx.Button(self, wx.ID_OK, _("OK"), dPos, (60, 26), 0)
            self.cancel_button = wx.Button(
                self, wx.ID_CANCEL, _("Cancel"), dPos, (60, 26), 0
            )
        ...
        if True:  # Layout
            ...
            button_sizer.AddMany(
                [
                    (self.add_button, 0, wx.ALL, 5),
                    (self.edit_button, 0, wx.ALL, 5),
                    (self.remove_button, 0, wx.ALL, 5),
                    ((0, 0), 1, wx.EXPAND, 5),
                    (self.ok_button, 0, wx.ALL, 5),
                    (self.cancel_button, 0, wx.ALL, 5),
                ]
            ...
            main_sizer.AddMany(
                [
                    (self.contents_list, 1, wx.ALL | wx.EXPAND, 2),
                    (button_sizer, 0, wx.EXPAND, 3),
                ]
            )
            self.SetSizer(main_sizer)
    ...
    def OnScreen(self):
        """Show list."""
        try:
            self.screen_data = ["[%s] : [%s]" % (x, self.data[x]) for x in self.data]
    ...
    def add(self, event):
        """Add item."""
        dialog = ItemDialog(self, _("Add Command"), {"": ""})
        value = dialog.GetValue
        if value == {"": ""}:
            return
        elif value:
...
class ItemDialog(wx.Dialog):  # Polemos
    ...
    def __init__(self, parent, title, item):
        ...
        for x in item:
            name, command = x, item[x]

        if True:  # Contents
            boxName = wx.StaticBox(self, -1, _("Name"))
            boxCommand = wx.StaticBox(
                self, -1, _("Command (add %target% variable for target file)")
            )
            self.name = wx.TextCtrl(
                self, wx.ID_ANY, name, dPos, dSize, wx.TE_NO_VSCROLL
            )
            self.command = wx.TextCtrl(self, wx.ID_ANY, command, dPos, dSize, 0)
            self.status_text = wx.StaticText(
                self, wx.ID_ANY, "", dPos, dSize, wx.ALIGN_CENTRE
            )  # Polemos Todo: Show errors.
            self.status_text.Wrap(-1)
            self.ok_button = wx.Button(self, wx.ID_OK, _("OK"), dPos, dSize, 0)
            self.cancel_button = wx.Button(
                self, wx.ID_CANCEL, _("Cancel"), dPos, dSize, 0
            )
        ...
        if True:  # Layout
            ...
            command_field_sizer.Add(
                self.command, 0, wx.RIGHT | wx.LEFT | wx.DOWN | wx.EXPAND, 5
            )
            buttons_sizer = wx.BoxSizer(wx.HORIZONTAL)
            buttons_sizer.AddMany(
                [
                    (self.ok_button, 0, wx.ALL, 5),
                    (self.status_text, 1, wx.ALL, 7),
                    (self.cancel_button, 0, wx.ALL, 5),
                ]
            )
            main_sizer = wx.BoxSizer(wx.VERTICAL)
            main_sizer.AddMany(
                [
                    (name_field_sizer, 0, wx.EXPAND, 5),
                    (command_field_sizer, 0, wx.EXPAND, 5),
...
class RunDialog(wx.Dialog):  # Polemos
    ...
    def __init__(
        self, parent, style=wx.CAPTION | wx.DEFAULT_DIALOG_STYLE | wx.STAY_ON_TOP
    ):
        """Init."""
        wx.Dialog.__init__(
            self,
        ...
        if True:  # Content
            ...
            self.input_text = wx.TextCtrl(
                self, wx.ID_ANY, "", dPos, dSize, wx.TE_NO_VSCROLL
            )
            self.run_button = wx.Button(
                self, wx.ID_ANY, _("Execute Command"), dPos, dSize, 0
            )
            self.cancel_button = wx.Button(
                self, wx.ID_CANCEL, _("Cancel"), dPos, dSize, 0
            )
        ...
        if True:  # Layout
            ...
            button_sizer.AddMany(
                [
                    (self.run_button, 0, wx.ALL, 3),
                    ((0, 0), 1, 0, 3),
                    (self.cancel_button, 0, wx.ALL, 3),
                ]
            )
            main_sizer = wx.StaticBoxSizer(boxCommand, wx.VERTICAL)
            main_sizer.AddMany(
                [
                    (self.input_text, 0, wx.EXPAND | wx.ALL, 5),
                    (button_sizer, 0, wx.EXPAND, 3),
                ]
            )
            self.SetSizer(main_sizer)
        ...
        if conf.settings["last.custom.cmd"]:
            self.input_text.SetValue(conf.settings["last.custom.cmd"])
        self.Layout()
        self.Centre(wx.BOTH)
        self.timer_po()
        self.ShowModal()
    ...
    def OnRun(self, event):
        self.GetValue = conf.settings["last.custom.cmd"] = (
            self.input_text.GetValue().strip()
        )
        self.Destroy()
...
class UtilsDialog(
    wx.Dialog
):  # Polemos: so many things here... also moved from Utils.py
    """Dialog for creating/modifying utilities."""

    result = None

    def __init__(
        self, parent, pos=dPos, size=(400, 300), new=True, data=("", "", "", "")
    ):
        """Utilities Dialog."""
        wx.Dialog.__init__(
            self,
        ...
        if True:  # components
            ...
            self.fldProg = wx.TextCtrl(
                self.Panel, -1, value=data[1], style=wx.TE_READONLY
            )
            btnBrowse = button(
                self.Panel,
                id=-1,
            ...
            self.fldDesc = wx.TextCtrl(
                self.Panel, -1, style=wx.TE_MULTILINE, value=data[3]
            )
        ...
        if True:  # Layout
            ...
            sizerWin.AddMany(
                [
                    (txtProg),
                    (sizerProg, 0, wx.EXPAND),
                    (txtArguments),
                    (self.fldArguments, 0, wx.EXPAND),
                    (txtDesc),
                    (self.fldDesc, 2, wx.EXPAND),
    ...
    def OpenFile(self, event):  # Polemos fix
        ...
        dialog = wx.FileDialog(
            self, _("Choose the new utility."), "", "", "*.*", wx.FD_OPEN
        )
        if dialog.ShowModal() != wx.ID_OK:
            dialog.Destroy()
            return
...
class AdvLog(wx.Dialog):  # Polemos
    ...
    def __init__(self, parent, title, logname="Output.log", ruleset="Default"):
        ...
        if True:  # Layout
            ...
            btn_sizer.AddMany(
                [
                    (self.OK_btn, 0, wx.ALIGN_LEFT | wx.ALL, 5),
                    ((0, 0), 1, wx.EXPAND, 5),
                    (self.savelog_btn, 0, wx.ALIGN_RIGHT | wx.ALL, 5),
                ]
            )
            main_sizer = wx.BoxSizer(wx.VERTICAL)
            ...
            main_sizer.AddMany(
                [
                    (self.text_log, 1, wx.EXPAND | wx.ALL, 5),
                    (btn_sizer, 0, wx.EXPAND, 5),
                ]
            )
            self.SetSizer(main_sizer)
            main_sizer.Fit(self)
    ...
    def write(self, data, color="BLACK"):
        ...
        if not data.endswith("\n"):
            ...
            try:
                ...
                self.text_log.WriteText(unicode(self.lineok, errors="replace"))
            self.lineok = ""
    ...
    def chkRules(self, line, color):
        ...
        if self.ruleset == "TES3lint":
            ...
            if line.lstrip().startswith("No patching necessary."):
                color = "RED"
            elif line.lstrip().startswith("Multipatch: Scanning"):
                color = "BROWN"
            elif line.lstrip().startswith("A multipatch has been conjured"):
                color = "BROWN"
            elif line.lstrip().startswith("tes3cmd multipatch skipping"):
                color = "RED"
        elif self.ruleset == "Fixit":
            if line.lstrip().startswith("No patching necessary."):
                color = "RED"
            elif line.lstrip().startswith("Multipatch: Scanning Active Plugins..."):
                color = "BROWN"
            elif line.lstrip().startswith("CLEANING:"):
                color = "RED"
            elif line.lstrip().startswith("Cleaning Stats for"):
                color = "BROWN"
            elif line.lstrip().startswith("A multipatch has been conjured"):
                color = "BROWN"
            elif line.lstrip().startswith("tes3cmd multipatch skipping"):
    ...
    def color(self, clr):
        ...
        if clr == "RED":
            ...
            self.text_log.SetDefaultStyle(wx.TextAttr(wx.Colour(153, 0, 0)))
    ...
    def onUpdate(self, event):
        ...
        if not self.log_done:
            if self.OK_btn.IsEnabled:
                self.OK_btn.Disable()
            if self.savelog_btn.IsEnabled:
                self.savelog_btn.Disable()
        else:
            self.OK_btn.Enable()
...
class ArchiveExplorer(wx.Dialog):  # Polemos
    ...
    def __init__(self, package_data, advData=None, title=_("Install Plugin...")):
        ...
        if True:  # Contents
            self.archiveText = wx.StaticText(
                self,
                wx.ID_ANY,
                _("Contents of: %s" % archive),
                dPos,
                dSize,
                wx.ALIGN_CENTRE,
            )
            archBoxTitle = wx.StaticBox(
                self, wx.ID_ANY, _("Right click on a folder for options:")
            )
            self.archiveTree = wx.TreeCtrl(
                archBoxTitle, wx.ID_ANY, dPos, dSize, wx.TR_DEFAULT_STYLE
            )
            self.statusText = wx.StaticText(
                self,
                wx.ID_ANY,
                _("Please select the mod's root folder."),
                dPos,
                dSize,
        ...
        if True:  # Layout
            ...
            mainSizer.AddMany(
                [
                    (self.archiveText, 0, wx.EXPAND | wx.UP | wx.RIGHT | wx.LEFT, 5),
                    (archBoxSizer, 1, wx.EXPAND | wx.ALL, 5),
                    (buttonSizer, 0, wx.EXPAND, 5),
                ]
            )
            self.SetSizer(mainSizer)
            self.Layout()
    ...
    def treeFactory(self, item):
        ...
        for x in range(self.max_depth):
            try:
                treeItems.append(
                    self.archiveTree.GetItemText(self.archiveTree.GetItemParent(item))
                )
    ...
    def onMouseR(self, event=None):
        ...
        if event is not None:
            ...
            self.selectedItem = self.archiveTree.GetSelection()
        ...
        # Rules
        if not self.dataSet:
            if unsetmenu.IsEnabled():
                unsetmenu.Enable(False)
        else:
            if not unsetmenu.IsEnabled():
                unsetmenu.Enable()
        try:
            if self.selectedItem == self.dataItem:
                if setmenu.IsEnabled():
                    setmenu.Enable(False)
            elif self.selectedItem != self.dataItem:
                if not setmenu.IsEnabled():
                    setmenu.Enable(True)
        except:
            pass
        # Events
        ...
        for folder in self.folders:
            if folder[2] == 0:
                setattr(
                    self,
                    "%s%s" % (folder[1], 0),
                    self.archiveTree.AppendItem(self.root, folder[1]),
                )
                if self.max_depth - 1 != folder[2]:
                    self.archiveTree.SetItemHasChildren(
                        getattr(self, "%s%s" % (folder[1], 0))
                    )
        # Sort root level
        ...
        for child in children:
            ...
            try:
                setattr(
                    self,
                    "%s%s" % (child[1], child[2]),
                    self.archiveTree.AppendItem(
                        getattr(self, "%s%s" % (child[0], child[2] - 1)), child[1]
                    ),
                )
            except:
                pass  # Catch problematic attrs
            # Show if there are any children to the current level and cache all remaining levels
            if self.max_depth - 1 != child[2]:
                try:
                    self.archiveTree.SetItemHasChildren(
                        getattr(self, "%s%s" % (child[1], child[2]))
                    )
                except:
                    continue
                self.archiveTree.Expand(getattr(self, "%s%s" % (child[1], child[2])))
                if not self.archiveTree.IsExpanded(
                    getattr(self, "%s%s" % (child[1], child[2]))
                ):
                    self.archiveTree.SetItemHasChildren(
                        getattr(self, "%s%s" % (child[1], child[2])), False
                    )
                self.archiveTree.Collapse(getattr(self, "%s%s" % (child[1], child[2])))
                # Sort children
                self.archiveTree.SortChildren(
                    getattr(self, "%s%s" % (child[1], child[2]))
                )
...
class HelpDialog(wx.Dialog):  # Polemos
    ...
    def __init__(
        ...
        if True:  # Content
            ...
            self.indexPanel = wx.Panel(
                self.main, wx.ID_ANY, dPos, dSize, wx.TAB_TRAVERSAL
            )
            self.contentPanel = wx.Panel(
                self.main, wx.ID_ANY, dPos, dSize, wx.TAB_TRAVERSAL
            )
            # Content
            self.conText = wx.StaticText(
                self.indexPanel,
                wx.ID_ANY,
                _("Help Index:"),
                dPos,
            ...
            self.index = wx.TreeCtrl(
                self.indexPanel,
                wx.ID_ANY,
                dPos,
                dSize,
                wx.TR_HIDE_ROOT | wx.TR_NO_LINES,
            )
            self.indexInf = wx.StaticText(
                self.indexPanel,
                wx.ID_ANY,
                _("Navigate with double clicking."),
                dPos,
                dSize,
                0,
            )
            self.help = wx.html.HtmlWindow(
                self.contentPanel, wx.ID_ANY, dPos, dSize, wx.html.HW_SCROLLBAR_AUTO
            )
        if True:  # Theme
        ...
        if True:  # Layout
            ...
            indexSizer.AddMany(
                [
                    (self.conText, 0, wx.ALL | wx.EXPAND, 5),
                    (self.search, 0, wx.EXPAND | wx.RIGHT | wx.LEFT, 5),
                    (self.index, 1, wx.EXPAND | wx.TOP | wx.RIGHT | wx.LEFT, 5),
                    (self.indexInf, 0, wx.ALL | wx.EXPAND, 5),
                ]
            )
            contentSizer = wx.BoxSizer(wx.VERTICAL)
    ...
    def onMouseR(self, event):
        ...
        # Conditions
        if self.help.SelectionToText() == "":
            CopyItm.Enable(False)
    ...
    def onIndex(self, event):
        ...
        try:
            self.help.ScrollToAnchor(
                self.index.GetItemText(event.GetItem())[:3].replace(".", "").strip()
            )
        except:
            pass
    ...
    def setIndex(self):
        ...
        for x in self.helpData.splitlines():
            ...
            if last:
                ...
                if result:
                    indexRaw.append(result)
                last = False
                tmp = []
        # Add to index
        self.root = self.index.AddRoot(_("Contents:"))
        for x in indexRaw:
            self.index.AppendItem(self.root, x)
...
class ConfBackup(wx.Dialog):
    ...
    def __init__(self, parent=None):
        ...
        if True:  # Content
            ...
            self.intro = wx.StaticText(
                self,
                wx.ID_ANY,
                _("Use [Ctrl] + [Left Mouse Click] to select/unselect multiple items:"),
                dPos,
                dSize,
                0,
            )
            self.cnfList = wx.ListBox(
                self,
                wx.ID_ANY,
                dPos,
                dSize,
                cnfListChoices,
                wx.LB_ALWAYS_SB | wx.LB_EXTENDED,
            )
            # Buttons
            self.res_btn = wx.Button(
                self, wx.ID_ANY, _("Restore Selected"), dPos, wx.Size(160, 22), 0
            )
            self.del_btn = wx.Button(
                self, wx.ID_ANY, _("Delete Selected"), dPos, wx.Size(160, 22), 0
            )
            self.bck_btn = wx.Button(
                self,
                wx.ID_ANY,
                _("Backup Current Configuration"),
                dPos,
                wx.Size(180, 22),
                0,
            )
            self.cnl_btn = wx.Button(
                self, wx.ID_CANCEL, _("Exit"), dPos, wx.Size(55, 22), 0
            )

        if True:  # Theming
            self.intro.Wrap(-1)
            self.cnfList.SetFont(
                wx.Font(
                    10,
                    wx.FONTFAMILY_DEFAULT,
                    wx.FONTSTYLE_NORMAL,
        ...
        if True:  # Layout
            ...
            contentSizer.AddMany(
                [
                    (self.intro, 0, wx.TOP | wx.RIGHT | wx.LEFT, 5),
                    (self.cnfList, 1, wx.EXPAND | wx.ALL, 5),
                ]
            )
            btnSizer = wx.BoxSizer(wx.HORIZONTAL)
            btnSizer.AddMany(
                [
                    (self.res_btn, 0, wx.LEFT | wx.RIGHT, 5),
                    ((0, 0), 1, 0, 5),
                    (self.del_btn, 0, wx.LEFT | wx.RIGHT, 5),
                    ((0, 0), 1, 0, 5),
                    (self.bck_btn, 0, wx.RIGHT | wx.LEFT, 5),
                    (self.cnl_btn, 0, wx.LEFT | wx.RIGHT | wx.DOWN, 5),
                ]
            )
            mainSizer = wx.BoxSizer(wx.VERTICAL)
            mainSizer.AddMany(
                [
                    (contentSizer, 1, wx.EXPAND, 5),
                    (btnSizer, 0, wx.ALIGN_RIGHT | wx.EXPAND, 5),
                ]
            )
            self.SetSizer(mainSizer)
            self.Layout()
    ...
    def onUpdate(self, event):
        ...
        if len(self.cnfList.GetSelections()) > 1 or not self.cnfList.GetSelections():
            if self.res_btn.IsEnabled():
                self.res_btn.Disable()
        else:
            if not self.res_btn.IsEnabled():
                self.res_btn.Enable()
        if not self.cnfList.GetSelections():
            if self.del_btn.IsEnabled():
        ...
        if not os.path.isdir(self.confBckDir):  # One final check...
            ...
            if not os.path.isdir(self.confBckDir):  # Failed.
                ErrorMessage(
                    None,
                    _(
                        "Access Denied: Unable to create a folder storage for backups!!\n\n"
                        "To proceed you neeed to manually create the following folder first:\n%s"
                        % self.confBckDir
                    ),
                )
                self.OnExit(None)
    ...
    def confInit(self):
        ...
        if not self.openmw:
            self.confFiles = {
                "Morrowind.ini": os.path.join(conf.settings["mwDir"], "Morrowind.ini")
            }
        elif self.openmw:
            self.confFiles = {
                "OpenMW.cfg": os.path.join(conf.settings["openmwprofile"], "OpenMW.cfg")
            }
            if conf.settings["tes3mp"]:
                self.confFiles["pluginlist.json"] = conf.settings["TES3mpConf"]

    def setConfList(self):
        """Creates configuration files list."""
        self.chkBckDir()
        rawConfList = {
            x: os.path.join(self.confBckDir, x)
            for x in os.listdir(self.confBckDir)
            if not os.path.isdir(os.path.join(self.confBckDir, x))
        }
        rawConfIndex = sorted(
            [rawConfList[x] for x in rawConfList], key=os.path.getmtime, reverse=True
        )
        sortedRawConfList = [
            x.replace(self.confBckDir, "").replace("\\", "") for x in rawConfIndex
        ]
        if not self.openmw:  # Morrowind
            ...
            ConfList = [
                x
                for x in sortedRawConfList
                if x.endswith(".cfg") or x.endswith(".json")
            ]
        return [
            x for x in ConfList if x.split("]")[1].lower() in self.confFileNamesTemplate
        ]  # One last check.
    ...
    def restore(self, event):
            ...
            if conf.settings["mwDir"]:  # Not really needed though...
                ...
                WarningMessage(
                    ...
                    _(
                        ...
                        "Please open the settings window and check the configuration."
                    ),
                )
                return
            ...
            if conf.settings["openmwprofile"]:
                ...
                WarningMessage(
                    ...
                    _(
                        ...
                        "Please open the settings window and set the configuration of OpenMW."
                    ),
                )
                return
            ...
            if conf.settings["TES3mpConf"]:
                ...
                WarningMessage(
                    ...
                    _(
                        ...
                        "Please open the settings window and set the configuration of TES3mp."
                    ),
                )
                return
        # File actions.
        try:
            ...
            ErrorMessage(
                self,
                _("Access Denied, unable to overwrite destination:\n\n%s") % err,
                _("Access Denied"),
            )
        except Exception as err:  # Who knows.
            ErrorMessage(
                self, _("An error occurred while trying to restore:\n\n%s" % err)
            )
        ...
        for fl in self.confFiles:
            ...
            try:
                ...
                ErrorMessage(
                    self,
                    _(
                        "An error occurred while trying to backup configuration:\n\n%s"
                        % err
                    ),
                )
        ...
        for fl in self.cnfList.GetSelections():
            ...
            try:
                ...
                ErrorMessage(
                    self,
                    _(
                        "An error occurred while trying to delete the selected item(s):\n\n%s"
                        % err
                    ),
                )
        self.cnfList.SetItems(self.setConfList())
