...
class TES3lint_Settings(
    wx.Dialog
):  # Polemos: a new settings window for TES3lint (Maybe I should also create a standalone program out of this).
    ...
    def __init__(self, parent, pos):
        ...
        if True:  # Box Sizers
            perl_sizer = wx.StaticBoxSizer(
                wx.StaticBox(self, wx.ID_ANY, _("Perl Executable:")), wx.HORIZONTAL
            )
            tesl3int_sizer = wx.StaticBoxSizer(
                wx.StaticBox(self, wx.ID_ANY, _("TES3lint Script Location:")),
                wx.HORIZONTAL,
            )
            custom_flags_teslint_sizer = wx.StaticBoxSizer(
                wx.StaticBox(self, wx.ID_ANY, _("Custom Flags:")), wx.VERTICAL
            )
            extras_teslint_sizer = wx.StaticBoxSizer(
                wx.StaticBox(self, wx.ID_ANY, _("Extra Options (May cause freezes):")),
                wx.VERTICAL,
            )
            result_sizer = wx.StaticBoxSizer(
                wx.StaticBox(self, wx.ID_ANY, _("Final Command:")), wx.HORIZONTAL
            )

        if True:  # Content
            # Perl Field/Button:
            self.perl_field = wx.TextCtrl(
                perl_sizer.GetStaticBox(), wx.ID_ANY, "", dPos, dSize, wx.TE_NO_VSCROLL
            )
            self.browse_perl_btn = wx.Button(
                perl_sizer.GetStaticBox(), wx.ID_ANY, "...", dPos, dSize, 0
            )
            # TES3lint Field/Button:
            self.tes3lint_field = wx.TextCtrl(
                tesl3int_sizer.GetStaticBox(),
                wx.ID_ANY,
                "",
                dPos,
                dSize,
            ...
            self.browse_teslint_btn = wx.Button(
                tesl3int_sizer.GetStaticBox(), wx.ID_ANY, "...", dPos, dSize, 0
            )
            # Recommended Flags:
            flags_radio_boxChoices = [
                _('-n  "normal" output flags on (fastest)'),
                _(' -r  "recommended" output flags on (slow)'),
                _("-a  all output flags on. (slowest)"),
                _(' -f "flags" specify flags below (separated by comma):'),
            ]
            self.flags_radio_box = wx.RadioBox(
                self,
                wx.ID_ANY,
                "Recommended Lists of Flags:",
                dPos,
                dSize,
                flags_radio_boxChoices,
                1,
            ...
            self.custom_flags_text = wx.TextCtrl(
                custom_flags_teslint_sizer.GetStaticBox(), wx.ID_ANY, "", dPos, dSize, 0
            )
            # Extra Options:
            self.debug_checkBox = wx.CheckBox(
                extras_teslint_sizer.GetStaticBox(),
                wx.ID_ANY,
                _('-D  "debug" output (vast)'),
                dPos,
                dSize,
                0,
            )
            self.verbose_checkBox = wx.CheckBox(
                extras_teslint_sizer.GetStaticBox(),
                wx.ID_ANY,
                _(' -v  "verbose" (possibly more output)'),
                dPos,
                dSize,
                0,
            )
            # TES3lint result:
            self.final_static = wx.StaticText(
                result_sizer.GetStaticBox(), wx.ID_ANY, "", dPos, dSize, 0
            )
        ...
        if True:  # Layout
            perl_sizer.AddMany(
                [(self.perl_field, 1, wx.ALL, 5), (self.browse_perl_btn, 0, wx.ALL, 5)]
            )
            tesl3int_sizer.AddMany(
                [
                    (self.tes3lint_field, 1, wx.ALL, 5),
                    (self.browse_teslint_btn, 0, wx.ALL, 5),
                ]
            )
            custom_flags_teslint_sizer.Add(
                self.custom_flags_text, 0, wx.ALL | wx.EXPAND, 5
            )
            extras_teslint_sizer.AddMany(
                [
                    (self.debug_checkBox, 0, wx.ALL, 5),
            ...
            buttons_sizer.AddMany(
                [(self.ok_btn, 0, wx.ALL, 5), space, (self.cancel_btn, 0, wx.ALL, 5)]
            )
            main_sizer = wx.BoxSizer(wx.VERTICAL)
            main_sizer.AddMany(
                [
                    (perl_sizer, 0, wx.EXPAND, 5),
                    (tesl3int_sizer, 0, wx.EXPAND, 5),
                    (self.flags_radio_box, 0, wx.ALL | wx.EXPAND, 5),
    ...
    def import_settings(self):
        ...
        self.custom_flags_text.SetValue(
            (",".join((unicode(x) for x in settings[1]))).strip("[ ]")
        )
    ...
    def export_settings(self):
        ...
        conf.settings["tes3lint.last"] = [
            self.flags_radio_box.GetSelection(),
            self.getFlags(),
    ...
    def onUpdate(self, event):
        ...
        if self.flags_radio_box.GetSelection() == 3:
            ...
            self.switch(False)
        if conf.settings["tes3lint.refresh"]:
            self.final_static.SetLabelText(
                "%s %s" % (self.cmd_factory(), _('"target_file"'))
            )

    def perl_dir(self, event):
        """..."""
        self.perl_field.SetValue(
            self.FileDialog("Perl", "Executable files (*.exe)|*.exe", "perl.exe")
        )

    def tes3lint_dir(self, event):
        """..."""
        self.tes3lint_field.SetValue(
            self.FileDialog("TES3lint", "All files (*.*)|*.*", "tes3lint")
        )
