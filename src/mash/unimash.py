...
from .cemetery import singletons
...
if locale.getlocale() == (None, None):  # Todo: Pos for Phoenix
    try:
        locale.setlocale(
            locale.LC_ALL, ""
        )  # Polemos: Possible fix for "locale.Error: unsupported locale setting"
    except:
        locale.setlocale(locale.LC_ALL, "en_US.UTF-8")  # Statistics...
language = locale.getlocale()[0].split("_", 1)[0]
if language.lower() == "german":
    language = "de"  # --Hack for German speakers who aren't 'DE'.
languagePkl, languageTxt = (
    os.path.join("locale", language + ext) for ext in (".pkl", ".txt")
)

# --Recompile pkl file?
if os.path.exists(languageTxt) and (
    not os.path.exists(languagePkl)
    or (os.path.getmtime(languageTxt) > os.path.getmtime(languagePkl))
):
