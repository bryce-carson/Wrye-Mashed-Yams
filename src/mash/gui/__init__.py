...
class List(wx.Panel):  # Polemos: Additions.
    ...
    def __init__(self, parent, id=-1, ctrlStyle=(wx.LC_REPORT | wx.LC_SINGLE_SEL)):
        ...
        if interface.style["lists.font.color"] is not None:
            [
                self.list.SetItemTextColour(x, interface.style["lists.font.color"])
                for x in xrange(self.list.GetItemCount())
            ]
            self.fontDefaultColor = interface.style["lists.font.color"]
        else:
            self.fontDefaultColor = wx.SystemSettings.GetColour(
        ...
        if conf.settings["interface.lists.color"]:
            if event.Moving():
                ...
                if mouseItem != self.mouseItem and mouseItem != -1:
                    ...
                    itemColor = (
                        self.list.GetItemTextColour(mouseItem)
                        if self.list.GetItemTextColour(mouseItem) != wx.NullColour
                        else self.fontDefaultColor
                    )
                    self.lastItemColor = (mouseItem, itemColor)
                    self.MouseEffect(mouseItem)
            elif (
                event.Leaving() and self.mouseItem is not None and self.mouseItem != -1
            ):
                try:
    ...
    def PopulateColumns(self):  # Polemos: added list alignments per col name.
        ...
        for colDex in xrange(self.numCols):
            ...
            if colName in right_align_po:
                wxListAlign = wx.LIST_FORMAT_RIGHT
            if colName in center_align_po:
                wxListAlign = wx.LIST_FORMAT_CENTER
    ...
    def GetSelected(self):
        ...
        itemDex = -1
        while True:
            itemDex = self.list.GetNextItem(
                itemDex, wx.LIST_NEXT_ALL, wx.LIST_STATE_SELECTED
            )
            if itemDex == -1:
                break
            else:
...
class NotebookPanel(wx.Panel):
    ...
    def enableFileDragDrop(
        self, window
    ):  # todo: Polemos, disabled for now, the implementation conflicts with drag and drop
