...
def profilePaths():
    ...
    profiledir = os.path.join(
        singletons.MashDir, "Profiles", conf.settings["profile.active"]
    )
    ...
    def checkactive(self):
        ...
        for x in profile:
            if not os.path.exists(x):
                return False  # todo: add dialog to inform user
        return True
    ...
    def createdefault(self):
        ...
        try:
            ...
            if not os.path.isfile(self.profile):
                self.create(self.profile, text=self.defaultini())
            if not os.path.isfile(self.mods):
                self.create(self.mods)
            if not os.path.isdir(self.confBcks):
                os.makedirs(self.confBcks)
        except:
            pass  # todo: add dialog to inform user
