...
class cleanop(wx.Dialog):  # Polemos: This is not implemented.
    ...
    def __init__(self, parent, style=wx.DEFAULT_DIALOG_STYLE | wx.STAY_ON_TOP):
        ...
        wx.Dialog.__init__(
            ...
            parent,
            id=wx.ID_ANY,
            title=_("tes3cmd Clean"),
            pos=dPos,
            size=wx.Size(429, 217),
            style=style,
        )
        self.SetSizeHintsSz(dSize, dSize)
        if True:  # Content
            self.clean_box0 = wx.CheckBox(
                self,
                wx.ID_ANY,
                _("clean cell subrecords AMBI,WHGT duped from masters"),
                dPos,
                dSize,
                0,
            )
            self.clean_box1 = wx.CheckBox(
                self,
                wx.ID_ANY,
                _("clean other complete records duped from masters"),
                dPos,
                dSize,
                0,
            )
            self.clean_box2 = wx.CheckBox(
                self, wx.ID_ANY, _("clean Evil GMSTs"), dPos, dSize, 0
            )
            self.clean_box3 = wx.CheckBox(
                self,
                wx.ID_ANY,
                _("clean object instances from cells when duped from masters"),
                dPos,
                dSize,
                0,
            )
            self.clean_box4 = wx.CheckBox(
                self,
                wx.ID_ANY,
                _("clean junk cells (no new info from definition in masters)"),
                dPos,
                dSize,
                0,
            )
            self.cancel_btn = wx.Button(self, wx.ID_CANCEL, _("Cancel"), dPos, dSize, 0)
            self.clean_selected_btn = wx.Button(
                self, wx.ID_ANY, _("Clean Selected"), dPos, dSize, 0
            )
            [
                x.SetValue(True)
                for x in (
        ...
        if True:  # Sizers
            ...
            button_sizer.AddMany(
                [
                    ...
                    (self.clean_selected_btn, 0, wx.ALL, 5),
                ]
            )
            main_sizer.SetMinSize(wx.Size(400, -1))
            main_sizer.AddMany(
                [
                    (self.clean_box0, 0, wx.ALL, 5),
                    (self.clean_box1, 0, wx.ALL, 5),
                    (self.clean_box2, 0, wx.ALL, 5),
                    (self.clean_box3, 0, wx.ALL, 5),
                    (self.clean_box4, 0, wx.ALL, 5),
                    (button_sizer, 1, wx.EXPAND, 5),
                ]
            )
...
class cleaner(wx.Frame):  # Polemos
    ...
    def __init__(
        ...
        wx.Frame.__init__(
            ...
            parent,
            id=wx.ID_ANY,
            title=_("tes3cmd Cleaner"),
            pos=dPos,
            size=wx.Size(530, 350),
            style=style,
        )
        mCleanedModsChoices = []
        self.SetSizeHintsSz(dSize, dSize)
        if True:  # Content
            ...
            # Buttons
            self.mSkip = wx.Button(
                self.main_panel, wx.ID_ANY, _("Skip"), dPos, (70, 26), 0
            )
            self.mStop = wx.Button(
                self.main_panel, wx.ID_ANY, _("Stop"), dPos, (70, 26), 0
            )
            self.save_log_btn = wx.Button(
                self.main_panel, wx.ID_ANY, _("Save Log"), dPos, dSize, 0
            )
            ...
            # Mod list/actions
            self.mCleanedMods = wx.ListBox(
                self.main_panel, wx.ID_ANY, dPos, dSize, mCleanedModsChoices, 0
            )
            self.clean_mod_info_text = wx.StaticText(
                self.main_panel, wx.ID_ANY, _("Cleaning..."), dPos, dSize, 0
            )
            self.mStats = wx.StaticText(self.main_panel, wx.ID_ANY, "", dPos, dSize, 0)
            # Output
            self.output_panel = wx.Panel(
                self.details_book, wx.ID_ANY, dPos, dSize, wx.TAB_TRAVERSAL
            )
            self.details_book.AddPage(self.output_panel, _("Output"), True)
            self.mLog = wx.TextCtrl(
                self.output_panel,
                wx.ID_ANY,
                "",
                dPos,
        ...
        if True:  # Layout
            ...
            info_sizer.AddMany(
                [
                    (self.clean_mod_info_text, 0, wx.ALL, 5),
                    (self.mStats, 0, wx.ALL, 5),
                    (self.details_book, 1, wx.EXPAND | wx.ALL, 5),
                ]
            )
            # Details
            details_main_sizer = wx.BoxSizer(wx.HORIZONTAL)
            details_main_sizer.AddMany(
                [
                    (self.mCleanedMods, 0, wx.ALL | wx.EXPAND, 5),
                    (info_sizer, 1, wx.EXPAND, 5),
                ]
            )
            # Buttons
            button_Sizer0 = wx.BoxSizer(wx.HORIZONTAL)
            button_Sizer0.AddMany(
                [
                    (self.mSkip, 0, wx.ALL, 5),
                    (self.mStop, 0, wx.ALL, 5),
                    (self.mProgress, 1, wx.ALIGN_CENTER_VERTICAL | wx.ALL, 5),
                ]
            )
            button_sizer1 = wx.BoxSizer(wx.HORIZONTAL)
            button_sizer1.AddMany(
                [
                    (self.ok_btn, 0, wx.ALIGN_RIGHT | wx.ALL, 5),
                    ((0, 0), 1, wx.EXPAND, 5),
                    (self.save_log_btn, 0, wx.ALIGN_RIGHT | wx.ALL, 5),
                ]
            )
            # SubMain
            ...
            sub_main_sizer.AddMany(
                [
                    (button_Sizer0, 1, wx.EXPAND, 5),
                    (details_main_sizer, 1, wx.EXPAND, 5),
                    (button_sizer1, 1, wx.EXPAND, 5),
                ]
            )
            # Main
